...
const kanbanColumnStyle = {
  ...
  boxShadow: "0 1px 6px 0 #f0f1f3",
  flexShrink: 0,
  height: 'fit-content' // Added to prevent columns from collapsing
};
...
// A droppable container for each column
...
// A draggable card for each lead
...
export function LeadsListIndex() {
  ...
  let filteredLeads = leads.filter((lead) => filterLead(lead, filter.toLowerCase(), source, status, assigned));
  filteredLeads = sortLeads(filteredLeads, sortBy);

  const stages = [...new Set(initialLeads.map((lead) => lead.stage))].sort();
  const getLeadsByStage = (stage: string) => leads.filter((lead) => lead.stage === stage);

  // DND-Kit specific logic for drag and drop
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newLead = {
      ...
      id: newId,
      status: { 
        label: newLeadData.status,
        color: newLeadData.status === "New" ? "#2979ff" : (newLeadData.status === "Qualified" ? "#34a853" : "#4285f4") // Simple color logic
      }
    };
    ...
    >
      ...
      <div style={{ display: "flex", gap: 24, margin: "30px 0 23px 0" }}>
        <div style={boxStyle}>
          ...
          <div style={{ color: "#0bab64", fontSize: 12, marginTop: 4 }}>+5% from last month</div>
        </div>
        <div style={boxStyle}>
          <div style={{ color: "#888fac", fontSize: 13, marginBottom: 6 }}>New Leads This Month</div>
          <div style={{ fontWeight: 700, fontSize: 23 }}>7</div>
          <div style={{ color: "#0bab64", fontSize: 12, marginTop: 4 }}>Increased by 12%</div>
        </div>
        <div style={boxStyle}>
          <div style={{ color: "#888fac", fontSize: 13, marginBottom: 6 }}>Qualified Leads</div>
          <div style={{ fontWeight: 700, fontSize: 23 }}>
            {initialLeads.filter((l) => l.status.label === "Qualified").length}
          </div>
          <div style={{ color: "#0bab64", fontSize: 12, marginTop: 4 }}>Up from last week</div>
        </div>
        <div style={boxStyle}>
          <div style={{ color: "#888fac", fontSize: 13, marginBottom: 6 }}>Conversion Rate</div>
          <div style={{ fontWeight: 700, fontSize: 23 }}>
            {Math.round((initialLeads.filter((l) => l.status.label === "Qualified").length / initialLeads.length) * 100)}%
          </div>
          <div style={{ color: "#fa6a69", fontSize: 12, marginTop: 4 }}>Target: 20%</div>
        </div>
      </div>
            ...
            <form onSubmit={handleFormSubmit}>
              ...
              <div style={formGroupStyle}>
                ...
                <textarea
                  ...
                  onChange={handleFormChange}
                  style={{ ...inputStyle, minHeight: '80px', resize: 'vertical' as 'vertical' }}
                />
              </div>
