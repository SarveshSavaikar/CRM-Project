...
export function LeadsListIndex() {
  ...
  // --- EXPORT FUNCTION ---
  const handleExport = () => {
    const headers = ["Lead ID", "Name", "Source", "Assigned To", "Stage", "Status", "Notes"];
    ...
    reader.onload = (e) => {
      ...
      if (typeof text === 'string') {
        ...
        const importedLeads = rows.slice(1).map(row => {
          ...
          // This assumes a fixed order and structure for the imported CSV
          return {
            id: columns[0] || "",
            name: columns[1] || "",
            source: columns[2] || "",
            ...
            stage: columns[4] || "",
            status: columns[5] || "",
            notes: columns[6]?.replace(/^"|"$/g, '').replace(/""/g, '"') || "",
          };
        });
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif", minHeight: "100vh", padding: "28px" }}>
      ...
      {showImportForm && (
        <div style={formContainerStyle}>
          <div style={{ ...formModalStyle, width: '400px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h2 style={{ margin: 0, fontSize: '18px', fontWeight: 600, color: '#2d334a' }}>Import Leads from CSV</h2>
              <button onClick={() => setShowImportForm(false)} style={{ background: 'none', border: 'none', fontSize: '24px', cursor: 'pointer', color: '#a8b0c8', lineHeight: 1, }}> &times; </button>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
              <label style={{ ...inputStyle, textAlign: 'center', cursor: 'pointer', padding: '15px', border: '2px dashed #dbe4f3', background: '#f7fafd' }}>
                <div style={{ color: '#636b91', fontSize: '14px', fontWeight: 500 }}>
                  Click to select a CSV file
                </div>
                <input type="file" accept=".csv" onChange={handleImport} style={{ display: 'none' }} />
              </label>
              <div style={{ color: '#888fac', fontSize: '12px' }}>
                Please ensure your CSV has the following columns: Lead ID, Name, Source, Assigned To, Stage, Status, Notes.
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, marginTop: 15 }}>
              <button type="button" onClick={() => setShowImportForm(false)} style={cancelButton}>Cancel</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div style={{ padding: "16px 18px" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 15 }}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <button style={{ borderRadius: 4, padding: "8px 16px", background: "#f7fafd", color: "#5167ad", border: "1px solid #dbe4f3", fontWeight: 500, fontSize: 15, cursor: "pointer", whiteSpace: "nowrap" }} onClick={() => setShowImportForm(true)}>Import</button>
                <button style={{ borderRadius: 4, padding: "8px 16px", background: "#f7fafd", color: "#5167ad", border: "1px solid #dbe4f3", fontWeight: 500, fontSize: 15, cursor: "pointer", whiteSpace: "nowrap" }} onClick={handleExport}>Export</button>
              </div>
            </div>
